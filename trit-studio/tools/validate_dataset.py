...
def validate(write_report=True):
    ...
    reports, overlaps = {}, []
    for split, label in [('seed', 'train'), ('validation', 'validation'), ('test', 'test'), ('challenge', 'test'), ('challenge-v4', 'test'), ('challenge-v5', 'test'), ('challenge-v6', 'test'), ('challenge-v7', 'test'), ('challenge-v8', 'test'), ('challenge-v9', 'test'), ('challenge-v10', 'test'), ('challenge-v11', 'test'), ('challenge-v12', 'test'), ('challenge-v13', 'test'), ('challenge-v14', 'test'), ('pretrain', 'train')]:
        ...
        for i, row in enumerate(rows):
            assert row['split'] == label and row['origin'] in ('authored-synthetic-v2','authored-synthetic-v3','authored-synthetic-v4','authored-synthetic-v5','authored-synthetic-v6','authored-synthetic-v7','authored-synthetic-v8','authored-synthetic-v9','authored-synthetic-v10','authored-synthetic-v11','authored-synthetic-v12','authored-synthetic-v13','authored-synthetic-v14')
            value = canonical(row)
    ...
    report = {'status':'passed', 'scope':'Data structure, integrity, disjointness and sequence budgeting; NOT a model quality evaluation',
              ...
              'frozen_v2_test_unchanged':True, 'frozen_v3_challenge_unchanged':True, 'frozen_v4_challenge_unchanged':True, 'frozen_v5_challenge_unchanged':True, 'frozen_v6_challenge_unchanged':True, 'frozen_v7_challenge_unchanged':True, 'frozen_v8_challenge_unchanged':True, 'frozen_v9_challenge_unchanged':True, 'frozen_v10_challenge_unchanged':True, 'frozen_v11_challenge_unchanged':True, 'frozen_v12_challenge_unchanged':True, 'frozen_v13_challenge_unchanged':True, 'baseline_v7_unchanged':True, 'cross_split_input_context_duplicates':0, 'total_examples':sum(x['count'] for x in reports.values())}
    if write_report:
        (ROOT/'reports').mkdir(exist_ok=True)
        (ROOT/'reports/dataset-audit-v14.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    return report
