...
public class BPETokenizer
{
    ...
    /// </summary>
    public string FormatPretraining(string text)
    {
        return text.Trim();
    }
    ...
    {
        writer.Write("BPE_COMPACT_V1");

        // Special tokens весят мало, но так формат безопаснее.
        writer.Write(SpecialTokens.Count);

        foreach (var s in SpecialTokens)
            writer.Write(s);

        // Главное: сохраняем только merge-пары как ID.
        writer.Write(Merges.Count);

        foreach (var (a, b) in Merges)
        {
            if (!Vocab.TryGetValue(a, out int aId) || !Vocab.TryGetValue(b, out int bId))
            {
                throw new InvalidDataException(
                    $"Merge pair not found in vocab: '{a}' + '{b}'");
            }

            WriteVarInt(writer, aId);
            WriteVarInt(writer, bId);
        }
    }
    ...
    public static BPETokenizer LoadFromStream(BinaryReader reader)
    {
        ...
        if (marker != "BPE_COMPACT_V1")
            throw new InvalidDataException(
                $"Неверный формат токенизатора: {marker}");

        int specialCount = reader.ReadInt32();

        var specials = new List<string>(specialCount);

        for (int i = 0; i < specialCount; i++)
            specials.Add(reader.ReadString());

        var tokenizer = new BPETokenizer(specials);

        tokenizer.SpecialTokens = specials;
        tokenizer.Vocab = new Dictionary<string, int>();
        tokenizer.Merges = new List<(string, string)>();

        var tokensById = new List<string>();

        int idx = 0;

        // 1. Special tokens
        foreach (var st in specials)
        {
            tokenizer.Vocab[st] = idx;
            tokensById.Add(st);
            idx++;
        }

        // 2. 256 byte tokens
        foreach (int b in BuildByteOrder())
        {
            string tok = ByteToUnicode[(byte)b].ToString();

            if (!tokenizer.Vocab.ContainsKey(tok))
            {
        ...
        // 3. Learned tokens from merges
        int mergeCount = reader.ReadInt32();

        for (int i = 0; i < mergeCount; i++)
        {
            int aId = ReadVarInt(reader);
            int bId = ReadVarInt(reader);

            if ((uint)aId >= (uint)tokensById.Count ||
                (uint)bId >= (uint)tokensById.Count)
            {
                throw new InvalidDataException(
                    $"Compact BPE: merge {i} ссылается на несуществующий токен.");
            }

            string a = tokensById[aId];
            string b = tokensById[bId];
            string merged = a + b;

            if (tokenizer.Vocab.ContainsKey(merged))
            {
                throw new InvalidDataException(
                    $"Compact BPE: дублирующийся merge-токен '{merged}' на merge {i}.");
            }

            tokenizer.Vocab[merged] = idx;
            tokensById.Add(merged);
            idx++;
    ...
    private static List<int> BuildByteOrder()
    {
        var byteOrder = new List<int>();
